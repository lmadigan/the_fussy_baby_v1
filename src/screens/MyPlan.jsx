...
export function MyPlan({ navigate }) {
    ...
    return (
      <Screen eyebrow="Free planning" title="My Plan">
        <Card>
          ...
          <div style={{ fontSize: "var(--type-title-size)", fontWeight: 700, color: "var(--text-primary)" }}>Choose a Playbook step</div>
  ...
  return (
    <Screen eyebrow={protocolPosition(protocol)} title="My Plan">
      <Card>
        <SectionLabel right={plan.status === "reviewed" ? "Reviewed" : "In progress"}>Current protocol</SectionLabel>
      ...
      <Card>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontSize: "25px", fontWeight: 600, color: "var(--text-primary)" }}>{formatShort(plan.reviewDate)}</div>
      ...
      {outcome && (
        <Card style={{ borderColor: "var(--accent-signal)" }}>
          ...
          <div style={{ fontSize: "var(--type-title-size)", fontWeight: 700, color: "var(--text-primary)" }}>{OUTCOME_LABELS[outcome.result]}</div>
